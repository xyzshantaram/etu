...
export const config = new Command('config')
    .description('Set/get configuration values. If neither key nor value is provided, prints the entire config.')
    .argument('[key]', `Config key to set/get. Prints the current value if no value is supplied. Valid keys: ${validKeys.join(',')}`)
